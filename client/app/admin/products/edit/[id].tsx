...
export default function EditProduct() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Toast.show({
                type: 'error',
                text1: 'Ошибка обновления товара',
                text2: error.response?.data?.message || "Что-то пошло не так"
            });
        } finally {
            setSubmitting(false);
    ...
    return (
        <ScrollView className="flex-1 bg-surface p-4">
            <View className="bg-white p-4 rounded-xl border border-gray-100 mb-20">
                <Text className="text-secondary text-xs font-bold mb-1 uppercase">Название товара *</Text>
                <TextInput
                ...
                <Text className="text-secondary text-xs font-bold mb-1 uppercase">Цена (₽) *</Text>
                <TextInput
                    className="bg-surface p-3 rounded-lg mb-4 text-primary"
                ...
                <Text className="text-secondary text-xs font-bold mb-1 uppercase">Количество на складе</Text>
                <TextInput
                    className="bg-surface p-3 rounded-lg mb-4 text-primary"
                ...
                <Text className="text-secondary text-xs font-bold mb-1 uppercase">Размеры (через запятую)</Text>
                <TextInput
                    className="bg-surface p-3 rounded-lg mb-4 text-primary"
                ...
                <Text className="text-secondary text-xs font-bold mb-1 uppercase">Категория</Text>
                <TouchableOpacity
                    onPress={() => setModalVisible(true)}
                    className="bg-surface p-3 rounded-lg mb-4 flex-row justify-between items-center"
                >
                    <Text className="text-primary">{category || "Выберите категорию"}</Text>
                    ...
                    <TouchableWithoutFeedback onPress={() => setModalVisible(false)}>
                        <View className="flex-1 justify-end bg-black/50">
                            <View className="bg-white rounded-t-2xl p-4 max-h-[50%]">
                                <Text className="text-lg font-bold text-center mb-4">Выберите категорию</Text>
                                <FlatList
                                    data={CATEGORIES}
                ...
                <Text className="text-secondary text-xs font-bold mb-1 uppercase">Изображения</Text>
                <View className="mb-4">
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        ...
                        {(existingImages.length + newImages.length) < 5 && (
                            ...
                            >
                                ...
                                <Text className="text-xs text-secondary mt-1">Добавить</Text>
                            </TouchableOpacity>
                        )}
                    </ScrollView>
                </View>

                <Text className="text-secondary text-xs font-bold mb-1 uppercase">Описание</Text>
                <TextInput
                ...
                <View className="flex-row justify-between items-center mb-6">
                    <Text className="text-primary font-bold">Рекомендуемый товар</Text>
                    <Switch
                        value={isFeatured}
                ...
                >
                    {submitting ? (
                        ...
                        <Text className="text-white font-medium text-lg">Обновить товар</Text>
                    )}
                </TouchableOpacity>
            </View>
        </ScrollView>
